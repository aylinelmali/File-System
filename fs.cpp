...
int myFileSystem::create_file(char name[8], int size) {
  ...
  for(int i = 0; i < 16; i++){
    disk.seekg(point);
    disk.read((char*)&n, sizeof(idxNode));
    if(n.used == 0){
      found = 1;
      n.used = 1;
      strcpy(n.name, name);
  ...
  disk.seekp(point);
  disk.write((char*)&n, sizeof(idxNode));
  return 1;
}  // End Create
...
int myFileSystem::delete_file(char name[8]) {
  ...
  for(int i = 0; i < 16; i++){
    disk.seekg(point);
    disk.read((char*)&n, sizeof(idxNode));
    if(n.used == 1 && (strcmp(name, n.name) == 0)){
      found = 1;
      break;
    }
    point = point + 48;
...
int myFileSystem::read(char name[8], int blockNum, char buf[1024]) {
  ...
  int addr = n.blockPointers[blockNum];
